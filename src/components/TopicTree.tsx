...
export const TopicTree: React.FC<TopicTreeProps> = ({
      ...
      <div key={topic.id} className="relative">
        ...
        {level > 0 && (
          <div className="absolute left-0 top-0 h-full flex">
            ...
            <div className="w-6 flex justify-center relative">
              ...
              <div className="absolute top-6 left-3 w-3 h-px bg-border" />
              {!isLast && (
                <div className="w-px bg-border h-full top-6 absolute" />
              )}
            </div>
          </div>
        )}
        ...
        >
          <div className={`relative group rounded-xl transition-all duration-300 ${
            isSelected 
              ? 'bg-primary/10 border border-primary/30 border-l-4 border-l-primary shadow-sm' 
                ...
                {hasChildren && (
                  <button
                    ...
                    className={`flex-shrink-0 p-1 rounded-md transition-all duration-200 ${
                      isSelected ? 'hover:bg-primary/10' : 'hover:bg-muted'
                    }`}
                  >
                    {isExpanded ? (
                    ...
                    {isSearchResult(topic) && topic.similarity && (
                      <Badge 
                        ...
                        className={`text-xs shrink-0 ${
                          isSelected ? 'border-primary/50 bg-primary/10 text-primary' : 'border-primary/30 bg-primary/10 text-primary'
                        }`}
                      >
  ...
  return (
    <div className="space-y-2 p-2">
      {/* Tree container with simple background */}
